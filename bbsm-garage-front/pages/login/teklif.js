...
export default function Teklif() {
  ...
  const fetchTeklifListesi = async () => {
    setLoading(true);
    try {
      const response = await fetchWithAuth("http://13.51.175.13:4000/teklif", {
        method: 'GET',
        redirect: 'follow'
      });
  ...
  const silSecilenleri = async () => {
    ...
    try {
      const deleteRequests = secilenTeklifler.map(teklifId =>
        fetchWithAuth(`http://13.51.175.13:4000/teklif/${teklifId}`, { method: 'DELETE' })
      );
  ...
  const handleTeklifEkle = async (teklif) => {
    ...
    try {
      const [deleteResponse] = await Promise.all([
        fetchWithAuth(`http://13.51.175.13:4000/teklif/${teklif.teklif_id}`, {
          method: 'DELETE',
          headers: {
      ...
      if (deleteResponse.ok)
      {
          ...
          const [postResponse] = await Promise.all([
            fetchWithAuth('http://13.51.175.13:4000/card', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
