...
export default function Detay() {
  ...
  async function fetchData(teklif_id) {
    setLoading(true);
    try {
      const response = await fetchWithAuth(`http://13.51.175.13:4000/teklif/${teklif_id}`, {
        method: 'GET',
        headers: {
  ...
  const handleSaveCardInfo = async () => {
    ...
    try {
      const response = await fetchWithAuth(`http://13.51.175.13:4000/teklif/${detay_id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleSaveYapilanlar = async () => {
    ...
    try {
      const response = await fetchWithAuth(`http://13.51.175.13:4000/teklif/${detay_id}/yapilanlar`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
