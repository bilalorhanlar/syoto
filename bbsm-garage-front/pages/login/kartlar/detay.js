...
export default function Detay() {
  ...
  async function fetchData(card_id) {
    setLoading(true);
    try {
      const response = await fetchWithAuth(`http://13.51.175.13:4000/card/${card_id}/yapilanlar`, {
        method: 'GET',
        headers: {
  ...
  const handleDelete = async (id) => {
    setLoading(true);
    try {
      const response = await fetchWithAuth(`http://13.51.175.13:4000/yapilanlar/${id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleSaveCardInfo = async () => {
    ...
    try {
      const response = await fetchWithAuth(`http://13.51.175.13:4000/card/${detay_id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleSaveYapilanlar = async () => {
    ...
    try {
      const response = await fetchWithAuth(`http://13.51.175.13:4000/card/${detay_id}/yapilanlar`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
